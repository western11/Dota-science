...
The dataset is way too large for my potato laptop, so i need to split it only to 2k obs
```{r}
set.seed(1502)
train1.edit <- sample_n(train1.edit, 2000)
```
...
use.cate <- as.data.frame(matrix(unlist(use.cate),nrow = 2000), stringsAsFactors = F)
...
splitted <- initial_split(train1.clean, prop = 0.8)
...
tester1$predicted <- predict(glm.mod, type = "response", newdata = tester1)
...
```{r}
trainer1.x <- trainer1[,-12]
tester1.x <- tester1[,-c(12)]

trainer1.y <- trainer1[,12]
tester1.y <- tester1[,12]
```
...
As we can see, glm model is way more better than knn (accuracy 89.4 > 54.1). However, we'll try to improve both model to achive higher accuracy (or other metrics)
...
```{r warning=FALSE,message=FALSE}
# model new glm
glm.mod.2 <- step(glm.mod, direction = "backward")
```
...
we see that adding additional predictor doesn't help much. we'll try to change the K to find the best accuracy. i build a loop function to train knn from k=20 to k=60, build the confusion matrix, and store the accuracy. thanks to [this](https://idc9.github.io/stor390/notes/cross_validation/cross_validation.html#generate_some_synthetic_data) article for the idea

```{r}
# build the loop from k=20 until k=60 by 1
k_values <- seq(20,60,1)
...
for(i in 1:num_k){
  ...
  k <- k_values[i]
  # build knn model from given k in loop start from 20 until 60 k
...
acc.k.df %>% ggplot(aes(x = k, y= acc)) +
  ...
  scale_x_continuous(breaks = seq(20,60,2))
```

from the plot we know that K = 55 have the highest accuracy, then lets re-model our knn
```{r}
...
`oh no!` its actually not hahaha. every loop generate some `random` stuff which i have no idea about. so changing K not actually answer our problem. deal with it, lets conclude our stuff.
...
## conclusion
From the table above we know that **glm is way better than knn** even though i already input all variable and change the K in KNN. The glm with `backward` direction stepwise have the best Accuracy, F1, Recall, and Sensitivity but a little fewer than glm without step in Specificity and Precision. It'll always possible to have higher accuracy (or other metrics) if we try another classification model. We'll do that in the future.

The most common use case in prediction dota2 winner is for gambling/bet, right? regardless of the bad thing of gambling addiction, it is important to us to look at the precision value. We need to limit the number of false positive so that our investment (aka money in gambling) didn't get much wasted. So if we prioritize model with higher precision value, in this case, **we'll use Logistic Regression with backward stepwise model.**
